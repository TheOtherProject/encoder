...
const valueSchema = numberSchema;
...
export const makeSchema = Yup.object({
  orderType: numberSchema.min(1).max(3).required(requiredErrorStrFn),
  collateral: Yup.array(assetSchema).min(1),
  principal: Yup.array(assetSchema).min(1),
  interest: Yup.object({
    token: Yup.string()
      .matches(/^0x[a-fA-F0-9]{40}$/, "${path}: Not a valid ethereum address")
      .required(requiredErrorStrFn),
  ...
  term: numberSchema.required(requiredErrorStrFn),
